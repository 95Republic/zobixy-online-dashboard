        ...
        <form onSubmit={add}>
            <div className='flex flex-col mb-3 md:flex-row gap-4 w-full text-[#d0d2d6]'>
                <div className='flex flex-col w-full gap-1'>
                    ...
                    <input className='px-4 py-2 focus:border-indigo-500 
                        ...
                        rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.name} 
                ...
                <div className='flex flex-col w-full gap-1'>
                    ...
                    <input className='px-4 py-2 focus:border-indigo-500 
                        outline-none bg-[#6a5fdf] border border-slate-700 
                        rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.brand} 
            ...
            <div className='flex flex-col mb-3 md:flex-row gap-4 w-full text-[#d0d2d6]'>
                ...
                <div className='flex flex-col w-full gap-1 relative'>
                    ...
                    <input readOnly onClick={()=> setCatShow(!catShow)} 
                    className='px-4 py-2 focus:border-indigo-500 
                        outline-none bg-[#6a5fdf] border border-slate-700 
                        rounded-md text-[#d0d2d6]' onChange={inputHandle} value={category} 
                        type="text" id='category' placeholder='--select category--' />
                        
                        <div className={`absolute top-[101%] bg-[#475569] w-full transition-all ${catShow ? 'scale-100' : 'scale-0'}`}>
                                ...
                                <div className='pt-14'></div>
                                    <div className='flex justify-start items-start flex-col h-[200px] overflow-x-scrool'>
                                        {
                                            allCategory.map((c,i) => <span className={`px-4 py-2 hover:bg-indigo-500 
                                                hover:text-white hover:shadow-lg w-full cursor-pointer 
                                                ${category === c.name && 'bg-indigo-500'}`} onClick={() => {
                                                setCatShow(false)
                                                setCategory(c.name)
                                                setSearchValue('')
                                                setAllCategory(categorys)
                ...
                <div className='flex flex-col w-full gap-1'>
                    ...
                    <input className='px-4 py-2 focus:border-indigo-500 
                        outline-none bg-[#6a5fdf] border border-slate-700 
                        rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.stock} 
            ...
            <div className='flex flex-col mb-3 md:flex-row gap-4 w-full text-[#d0d2d6]'>
                ...
                <div className='flex flex-col w-full gap-1'>
                    ...
                    <input className='px-4 py-2 focus:border-indigo-500 
                        outline-none bg-[#6a5fdf] border border-slate-700 
                        rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.price} 
                ...
                <div className='flex flex-col w-full gap-1'>
                    ...
                    <input className='px-4 py-2 focus:border-indigo-500 
                        outline-none bg-[#6a5fdf] border border-slate-700 
                        rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.dicount} 
            ...
            <div className='flex flex-col w-full gap-1 mb-5'>
                ...
                <textarea className='px-4 py-2 focus:border-indigo-500 
                    outline-none bg-[#6a5fdf] border border-slate-700 
                    rounded-md text-[#d0d2d6]' onChange={inputHandle} value={state.description} 
                    type="text" name='description' id='description' placeholder='Description' cols="10" rows="4"></textarea>
            ...
            sm:gap-4 md:gap-4 gap-3 w-full text-[#d0d2d6] mb-4'> 
                {
                    imageShow.map((img,i) => <div className='h-[180px] relative'>
                        ...
                        hover:shadow-lg hover:shadow-slate-400/50 text-white absolute top-1 right-1 rounded-full'><IoMdCloseCircle/></span>
                    </div>)
                }
            ...
            <div className='flex'>
            <button disabled={loader ? true : false} className='bg-red-500 w-[280px] hover:shadow-red-300/50
                hover:shadow-lg text-white rounded-md px-7 py-2 mb-3'>
                    {
                        loader ? <PropagateLoader color='#ffff' cssOverride={overrideStyle} /> : 'Add Product'
                    }
