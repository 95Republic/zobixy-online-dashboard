...
const PaymentRequest = () => {
    ...
    const {successMessage, errorMessage,pendingWithdrawals,loader} = useSelector(state => state.payment)
        ...
        return (
            <div style={style} className='flex text-sm text-white font-medium'>
                ...
                <div className='w-[25%] p-2 whitespace-nowrap'>
                    ...
                    <button disabled={loader} onClick={() => confirm_request(pendingWithdrawals[index]?._id)} className='bg-indigo-500 shadow-lg hover:shadow-indigo-500/50 
                    px-3 py-[2px] cursor-pointer text-white rounded-sm text-sm'>{(loader && paymentId === pendingWithdrawals[index]?._id) ? 'loading...' : 'Confirm'}</button>
                </div>
